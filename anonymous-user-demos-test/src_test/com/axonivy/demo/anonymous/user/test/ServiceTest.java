package com.axonivy.demo.anonymous.user.test;
...
import com.axonivy.demo.anonymous.user.core.Service;
...
class ServiceTest {
  ...
  void appRelativeLink_registerProcess() {
    String baseUrl = Ivy.var().get("com.axonivy.demo.anonymous.user.baseUrl");
  ...
  void appRelativeLink_submitMissingDataWithToken() {
    ...
    String result = Service.get().appRelativeLink(
        "anonymous-user-demos-open/" + REGISTER_PROCESS_ID + "/submitMissingData.ivp?token=" + token);
    assertThat(result)
  ...
  void appRelativeLink_emailVerificationWithToken() {
    String token = UUID.randomUUID().toString();
    String result = Service.get().appRelativeLink(
        "anonymous-user-demos-open/" + REGISTER_PROCESS_ID + "/emailVerification.ivp?token=" + token);
    assertThat(result)
  ...
  void appRelativeLink_normalizesRedundantPathSegments() {
    String direct = Service.get().appRelativeLink("anonymous-user-demos-open/start.ivp");
    String withDots = Service.get().appRelativeLink("anonymous-user-demos-open/sub/../start.ivp");
