...
Open the Engine Cockpit at http://localhost:9080/. Note, that in this demo the Engine Cockpit will only be available om the internal network and not through the reverse proxy server.

Create a new application with the exact name `anonymous-user-demos` and deploy the `zip` file (or `iar` files) generated before. If necessary, release the application.
...
Make sure the e-mail host is configured to `mail` port `25`. Set the System Configuration `EMail.Server.Port` to port `25` and the `EMail.Server.Host` to `mail`.
...
Other than that, continue in the same way as in variant A.
